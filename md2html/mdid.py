...
def main(args):
    ...
    for i, line in enumerate(md):
        if line.startswith(u'#'):
            ...
            try:
                attrs['id'] = [a[1:]
                               for a in attrs_raw if a.startswith('#')][0]
            except IndexError:
                attrs['id'] = u''
            ...
            for other in others:
                ...
                if k in attrs.keys():
                    ...
                    attrs[k] = vals
            logger.debug("attrs:\n    {0}".format(u"\n    ".join(
                [u"{0}:'{1}'".format(k, v) for (k, v) in attrs.iteritems()])))
            ...
            if len(attrs['id']) == 0:
                ...
                if m is not None:
                    # ignores id specs greater than the first
                    xmlid = m.group(1)
                else:
                    xmlid = heading['text']
                try:
                    xmlid = RX_NOTPUNCT.search(xmlid).group(0).strip().lower()
                except AttributeError as inst:
                    raise AttributeError(inst.__str__(
                    ) + " while trying to create xmlid from '{0}'".format(heading['text']))
                words = xmlid.split()
            ...
            hdr_new = u' '.join(
                (heading['hash'], heading['text'], u'{:', attribs, u'}'))
            if hdr_new != line:
                logger.warning(
                    u"input line {0}: rewrote line as '{1}'".format(i, hdr_new))
...
if __name__ == "__main__":
    ...
    try:
        ...
        parser.add_argument("-i", "--input_filename",
                            type=str, help="markdown filename to read")
        parser.add_argument("-o", "--output_filename",
                            type=str, help="markdown filename to write")
